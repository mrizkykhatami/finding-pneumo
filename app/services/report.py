...
def build_report_pdf(scan, root_path: str) -> BytesIO:
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"FindingPneumo Screening Report {scan.patient_id}",
    )
    story = []
    ...
    header = Table(
        [[
            ...
            Paragraph(
                f"AI Pneumonia Screening Report<br/>"
                f"Scan No.: <b>#{scan.id}</b><br/>"
                f"Date: {scan.tanggal:%d %B %Y, %H:%M}" if scan.tanggal else "",
                ss["BrandSub"]),
        ]],
    ...
    p = scan.patient
    doctor = scan.doctor
    gender = {"L": "Male", "P": "Female"}.get(
        p.jenis_kelamin if p else None, "—")
    info = [
        ["Patient ID", p.id if p else "—", "Name", p.nama if p else "—"],
        ["Age", str(p.umur) if p and p.umur is not None else "—", "Gender", gender],
        ["Contact", (p.kontak or "—") if p else "—", "Examiner",
    ...
    story += [Paragraph("Patient Information", ss["H2"]), info_tbl, Spacer(1, 4)]

    if p and (p.gejala or p.riwayat_medis):
        story.append(Paragraph(
            f"<b>Symptoms:</b> {p.gejala or '—'}<br/><b>Medical history:</b> {p.riwayat_medis or '—'}",
            ss["Body"]))
    ...
    story += [Paragraph("AI Analysis Summary (3-Model Ensemble)", ss["H2"]),
    ...
    story += [Paragraph("Activation Maps (Grad-CAM)", ss["H2"]), grid, Spacer(1, 4)]
    ...
    story += [Paragraph("Doctor Notes & Interpretation", ss["H2"]), note_tbl, Spacer(1, 10)]
    ...
    story.append(Paragraph(
        "<b>Disclaimer:</b> This report is generated by an AI-assisted screening tool "
        "(FindingPneumo) and is intended for support only. The results do not replace clinical examination, "
        "diagnosis, or medical decision-making. Correlate with the patient’s clinical condition.",
        ss["Small"]))
...
def _footer(canvas, doc):
    ...
    canvas.drawString(18 * mm, 10 * mm,
                      f"FindingPneumo · Printed {datetime.now():%d %b %Y %H:%M}")
